...
export async function broadcastNewMessageToAdmins(
  ...
  // SSE and push run concurrently: a stalled SSE stream must never delay or
  // starve the push fan-out (a wedged writer once swallowed every admin push).
  const sseTask = (async () => {
    console.log(
  ...
  await Promise.all([sseTask, pushTask]);
}
