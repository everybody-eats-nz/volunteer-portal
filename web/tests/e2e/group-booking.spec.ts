...
test.describe.skip("Group Booking Feature", () => {
  ...
  test("should display group bookings in My Shifts page", async ({ page }) => {
    ...
    if (sectionExists) {
      ...
      await expect(
        groupSection.locator('[data-testid="group-bookings-title"]')
      ).toBeVisible();
      await expect(
        groupSection.locator('[data-testid="group-bookings-count"]')
      ).toBeVisible();

      // Check for group booking cards
      const groupCards = groupSection.locator(
        '[data-testid^="group-booking-"]'
      );
      ...
      if (cardCount > 0) {
        ...
        await expect(
          firstCard.locator('[data-testid="group-name"]')
        ).toBeVisible();
        ...
        if (await memberSection.isVisible()) {
          ...
          const registeredMembers = firstCard.locator(
            "text=/Registered Members/"
          );
          const pendingInvites = firstCard.locator(
            "text=/Pending Invitations/"
          );

          // At least one of these should be visible
          const hasMembers =
            (await registeredMembers.isVisible().catch(() => false)) ||
  ...
  test("should show registration status on group detail page", async ({
    page,
  }) => {
    await loginAsVolunteer(page);
    ...
    if (sectionExists) {
      ...
      if (await manageButton.isVisible()) {
        ...
        const hasSections =
          (await readyMembers.isVisible().catch(() => false)) ||
        ...
        const hasStatus =
          (await completeProfile.isVisible().catch(() => false)) ||
  ...
  test("unauthorized access to admin group features is blocked", async ({
    page,
  }) => {
    await loginAsVolunteer(page);
    ...
    if (loginUrl.includes("/login")) {
      test.skip(
        true,
        "Volunteer login failed - skipping unauthorized access test"
      );
    }
    ...
    expect(currentUrl).toMatch(
      /\/(login|dashboard|profile|shifts|admin|unauthorized|403|401|error)/
    );
  });
  ...
  test("group booking API endpoints are accessible", async ({ page }) => {
    ...
    const inviteResponse = await page.request.get(
      "/api/group-invitations/invalid-token"
    );
    expect(inviteResponse.status()).toBeLessThan(500); // Should handle gracefully
  });

  test("admin cannot approve group booking with pending invitations", async ({
    page,
  }) => {
    await loginAsAdmin(page);
    ...
    if (sectionExists) {
      ...
      const pendingGroup = groupSection
        .locator('[data-testid^="group-booking-card-"]')
        .filter({
      ...
      if (await pendingGroup.isVisible()) {
        ...
        const pendingWarning = pendingGroup.locator(
          "text=/Some invited members have not joined yet/"
        );

        if (await pendingWarning.isVisible()) {
          // Approve button should be disabled
          const approveButton = pendingGroup.locator("button", {
            hasText: /approve group/i,
          });
  ...
  test("admin can see pending invitations in group member list", async ({
    page,
  }) => {
    await loginAsAdmin(page);
    ...
    if (currentUrl.includes("/login")) {
      test.skip(
        true,
        "Admin login failed - skipping pending invitations display test"
      );
    }
    ...
    if (sectionExists) {
      ...
      const groupWithPending = groupSection
        .locator('[data-testid^="group-booking-card-"]')
        .filter({
      ...
      if (await groupWithPending.isVisible()) {
        ...
        const pendingInvite = groupWithPending
          .locator(".bg-amber-50")
          .filter({
        ...
        if (await pendingInvite.isVisible()) {
          ...
          await expect(
            pendingInvite.locator("text=awaiting response")
          ).toBeVisible();
        }
      }
    }
