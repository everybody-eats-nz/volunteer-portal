...
test.describe("Admin Command Palette", () => {
  ...
  test.describe("Accessibility and Visibility", () => {
    test("command palette trigger button is visible on all admin pages", async ({
      page,
    }) => {
  ...
  test.describe("User Search Functionality", () => {
    ...
    test("empty search shows navigation items only", async ({ page }) => {
      ...
      await expect(
        page.locator("[cmdk-group-heading]", { hasText: "Overview" })
      ).toBeVisible();
      await expect(
  ...
  test.describe("Navigation Features", () => {
    test("navigation items are visible and searchable", async ({ page }) => {
      ...
      await expect(
        commandDialog.locator("[cmdk-group-heading]", { hasText: "Overview" })
      ).toBeVisible();
    ...
    test("searching for navigation items filters correctly", async ({
      ...
      await expect(commandDialog.getByText("Dashboard")).toBeVisible();
      await expect(
        commandDialog.getByText("Overview and statistics")
      ).toBeVisible();
    });
    ...
    test("navigation items have descriptive subtitles", async ({ page }) => {
      ...
      await expect(
        page.getByText("Manage volunteers and admins")
      ).toBeVisible();
      await expect(page.getByText("Add new volunteer shifts")).toBeVisible();
      await expect(
        page.getByText("View and edit existing shifts")
      ).toBeVisible();
    });
