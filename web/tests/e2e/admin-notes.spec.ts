...
test.describe("Admin Notes Management", () => {
  ...
  test("should cancel adding a new note", async ({ page }) => {
    ...
    await expect(
      page.getByText("This note should be cancelled")
    ).not.toBeVisible();
  });
...
test.describe("Admin Notes on Shifts Page", () => {
  ...
  test("should display admin notes button for volunteers with notes on shifts page", async ({
    page,
  }) => {
    ...
    if (await firstButton.isVisible()) {
      ...
      const openProfileButton = page.getByTestId(
        "open-volunteer-profile-button"
      );
  ...
  test("should display correct note count on notes button", async ({
    page,
  }) => {
    await page.goto("/admin/shifts");
    await waitForPageLoad(page);
    ...
    const singleNoteButton = page.locator(
      'button:has-text("Note"):not(:has-text("notes"))'
    );
